...
import io.github.primelib.webmethodsapigateway4j.auth.AuthInterceptor;
...
public class APIGatewayFactory {
    ...
    public static <T> T create(Consumer<APIGatewayFactorySpec<T>> spec) {
        ...
        if (config.api().isInterface()) {
            ...
            try {
                APIGatewayApi api = buildClient(s -> {
                    s.api(APIGatewayApi.class);
                    s.applySpec(config);
                });
                return config.api().getConstructor(APIGatewayApi.class).newInstance(api);
            } catch (Exception ex) {
                throw new IllegalArgumentException("api must have a constructor with one parameter of type APIGatewayApi", ex);
            }
        }
    }
